...
## ▶ Atualização 2026-06-25 — tempo do gol (D-71) + cartões/escanteios (D-72) + mercados/UX
Dois mercados de **dado externo** a R$ 0 via StatsBomb LOCAL (cumpre a promessa da D-21). Detalhe: [[Tempo do gol + cartoes-escanteios (StatsBomb, 2026-06-25)]].
- **Tempo do gol (D-71) — PASSOU, no ar:** Poisson não-homogêneo sobre a curva empírica de minutos (`scm/timing.py`, `dados/goal_timing.json`; 314 jogos/771 gols, f45=0,401). Portão de calibração previsto×observado **±1,2 pp** (gol 1ºT 62,6/63,7; 2ºT 77,0/75,8; dois tempos 48,2/48,7; 0 gols 8,6/9,2). [verificado] Mercados: gol 1ºT/2ºT, nos dois tempos, 1º gol até X, faixa do 1º gol, **resultado no intervalo (HT)**. Amostra ampla: `scm.timing download` + `build-csv` (martj42).
- **Cartões/escanteios (D-72) — REPROVADO como previsão por seleção:** LOO MAE do total, modelo-time vs média-global — cartões **+0,3%** (sem sinal), escanteios **+1,7%** (ruído). [verificado] Publicado só como **histórico descritivo + over/under da média da competição** (calibrado: over 3,5 cart 48%, over 9,5 esc 42%), rotulado como não-previsão. `scm/setpiece.py`, `dados/setpiece.csv` (73 seleções). Mesma régua que barrou xG/DC/Glicko. Teto R$ 0 (só StatsBomb cobre seleção).
- **Também nesta sessão (sem D-número):** mercados de gols **expandidos** (win-to-nil, empate-anula, par/ímpar, total exato/faixas, handicap/margem, scorecast, grade de placar) — releituras da MESMA Poisson (D-21), sem portão; mercados reorganizados em **seções colapsáveis** (estilo casa de aposta); **botão claro/escuro** (`data-theme`) nas 4 telas, corrigindo hover e inputs ilegíveis no escuro.
- **Higiene/testes:** `tests/test_timing.py` (9) + `tests/test_setpiece.py` (6) + `test_markets.py` estendido (+6) — somas=100%, monotonia, baseline≠por-time, retrocompat. **25/25 passam** (shim de `approx`; rode `pytest` na máquina). Os testes acharam e corrigiram um **bug de truncamento** no `team_over` de `markets()` (cauda `pa[thr:]` → complemento exato `1−Σpa[0:thr]`).
- **Fechamento honesto:** tempo do gol é estável e passou; cartão/escanteio por seleção **não tem sinal** no dado grátis disponível. Flask não roda no sandbox → teste pelo `.bat`.

## ▶ Atualização 2026-06-26 — busca de melhorias (STEP 0 medido) + 2 ações operacionais
Análise céptica com **STEP 0 medido no snapshot** (não inventado): [[Busca de melhorias — analise PIT (2026-06-26)]]. Conclusão = **o núcleo segue no teto; o ganho seguro hoje é medir, não fórmula**. Único termo de modelo com sinal novo real = **mando por confederação** (CONMEBOL home +0,040 fora de altitude; UEFA −0,040, estável; corr c/ `dr` 0,02–0,18) — mas **impacto ~0 na Copa 2026** (sede neutra) → ganho de Brier histórico, não de previsão; fica como candidato ao portão (`calibrate_confed`, não implementado). Descanso/congestão **morto** (corr c/ resíduo 0,0005; 49 jogos major informativos; Copa tem calendário simétrico).
Implementadas as **duas ações que dependem só de código** (as outras são captura manual da linha de fechamento):
- **D-75 — ε do mata-mata fechado em 0,049 [MEDIDO]:** `calibrate_ko` sobre `shootouts.csv` (n=677): P(mais forte vence nos pênaltis)=0,549 → **ε̂=+0,049 IC95 Wilson [+0,012,+0,087]**; o `0,03` antigo caía dentro do IC. `PredictParams.eps_ko 0,03→0,049` (só-simulação, **sem rebuild, sem bump** do modelo de 1 jogo). Impacto no título microscópico (~3 disputas/Copa); gate de título é sub-powered → adoção por **calibração direta do parâmetro**, não por portão de Brier de avanço. [verificado: valor em `predictor.py`]
- **D-76 — `monitor` ganhou calibração + DRIFT POR MERCADO:** `monitor.reliability_by_market` (1X2 top-pick, Over 2.5, BTTS) sobre o registro imutável: previsto×observado, Brier vs taxa-base e **z-score binomial**. É **power-aware** — com n<10 marca `n baixo` e **não alarma** (não confunde ruído com deriva, mesma disciplina do portão). Instrumentação, **sem portão, sem versão**. Novo `tests/test_monitor.py` (5 casos: desfechos corretos, drift power-aware, flag DRIFT real, ok calibrado, registro vazio) — **9/9 asserts [verificado] em harness isolado** (o mount do sandbox trunca arquivos recém-editados na leitura → **rode `pytest -q` na sua máquina** p/ confirmar; quirk conhecido D-14/D-16).
- **D-77 — captura SEMI-AUTO da linha de fechamento (`scm/odds_close.py`):** resolve o "você esquece" sem furar o R$0 (não há feed grátis/confiável de odds da Copa; The Odds API cobra futebol, scraping é frágil/ToS). `watch` avisa (console + pop-up nativo do Windows, sem deps) ~5 min antes de cada jogo sem linha `close`; `set "Casa" "Fora" CASA EMPATE FORA` de-viga (reusa `scm.odds`) e grava `source='close'`; `due` lista os pendentes. Fixtures ganham campo `kickoff` ISO **opcional** (sem ele, avisa no dia). Launcher `Lembrete de odds (Copa).bat`. Instrumentação, sem portão. `tests/test_odds_close.py` (6 casos) — **15/15 asserts [verificado] em harness isolado**; rode `pytest` na máquina.
- **Próximo passo operacional (você):** abra o `Lembrete de odds (Copa).bat` no dia do jogo (idealmente preenchendo `kickoff` em `dados/fixtures.json`); ao ser avisado, cole as 3 odds. Aí `monitor` mede **CLV real** (modelo × fechamento). É a única peça sensível ao tempo — jogo que rola sem fechamento gravado é dado perdido.
- **D-78 — clone reprodutível (BUG de distribuição corrigido):** o `.gitignore` ignorava `dados/*.json` e `dados/*.csv` **sem exceção** → `git ls-files` confirmou que **só os `.example` estavam no repo**; quem clonava recebia dados ZERO e batia em "preencha dados/copa2026.json" (o sorteio não é baixável). Reescrito: `dados/` é **versionado por padrão**, ignorando só `*.sqlite` (regenerável, 49 MB) e `*.png`. Agora versionados: `copa2026.json`, `goal_timing.json`, `setpiece.csv`, `xg.csv`, `climatology.json`, `shootouts.csv` (curados/derivados) **+ snapshot martj42** `results.csv`/`goalscorers.csv` (congela os dados → resultados **idênticos** aos do autor; o build é determinístico e o Monte Carlo é seeded=12345). `.bat` ajustado: 1º arranque **builda do snapshot versionado, offline** (`ingest` sem `--download`), caindo p/ download só se faltar o `results.csv`. [verificado: regras testadas em repo git isolado; `ingest --csv` builda sem rede]. **Repo +~7,5 MB.** Atualizar dados depois: botão "Atualizar tudo" (ou `ingest --download`). Único requisito de internet no 1º run: `pip install` das deps.
- **Commit-ready** (rode na sua máquina; agora **inclui os dados** antes ignorados): `git add -A && git status` (confira copa2026.json/results.csv/goalscorers.csv como novos) `&& git commit -m "D-75/76/77/78: eps_ko=0.049 [medido] + monitor drift por mercado + odds_close (CLV) + clone reprodutivel (versiona snapshot+dados curados, .bat offline) + testes" && git push`
