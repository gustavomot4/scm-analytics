...
- **Próximo passo operacional (você, manual):** capturar a **linha de fechamento** de cada jogo restante (pré-kickoff) → `scm.odds ingest ... --source close`; aí `monitor` mede **CLV real**. É a única peça sensível ao tempo — jogo que rola sem fechamento gravado é dado perdido.
- **Commit-ready:** `git add -A && git commit -m "D-75/D-76: eps_ko=0.049 [medido, shootouts n=677] + monitor reliability/drift por mercado (power-aware) + test_monitor; analise de melhorias STEP 0"`
